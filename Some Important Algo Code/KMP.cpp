...
void Failure_Fun()
{
    int k = 0, len = strlen( pat + 1 ) + 1, p = 0; pre[ 1 ] = 0;
    for(int i = 2; i < len; i++)
    {
        while(p && pat[ i ] != pat[p + 1]) p = pre[ p ];
...
int KMP()
{
    int ret = 0, p = 0, len = strlen(taxt + 1) + 1, pat_len = strlen(pat + 1);
    for(int i = 1; i < len; i++)
    {
        while(p && taxt[ i ] != pat[p + 1]) p = pre[ p ];

        if(pat[p + 1] == taxt[ i ]) p++;
        if(p == pat_len) ret++, p = pre[ p ];
    }
    return ret;
}
